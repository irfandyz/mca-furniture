<footer class="footer bg-light pt-5 mt-5">
    <div class="container">
        <div class="row">
            <div class="col-12 col-sm-6 col-lg-4 mb-4">
                <h5 class="mb-4">Our Location</h5>
                <p class="mb-0">Jl. Suryanegara Blok Sidampul RT 04 RW 01</p>
                <p class="mb-0">Desa Pamijahan Kec. Plumbon Kab. Cirebon</p>
                <p>45159 - Indonesia</p>
            </div>

            <div class="col-12 col-sm-6 col-lg-4 mb-4">
                <h5 class="mb-4">Quick Link</h5>
                <ul class="list-unstyled">
                    <li class="mb-2"><a href="#" class="text-dark text-decoration-none hover-opacity">Home</a></li>
                    <li class="mb-2"><a href="#about" class="text-dark text-decoration-none hover-opacity">About Us</a></li>
                    <li class="mb-2"><a href="#product" class="text-dark text-decoration-none hover-opacity">Product</a></li>
                    <li class="mb-2"><a href="#service" class="text-dark text-decoration-none hover-opacity">Service</a></li>
                    <li><a href="#contact" class="text-dark text-decoration-none hover-opacity">Contact Us</a></li>
                </ul>
            </div>

            <div class="col-12 col-sm-6 col-lg-4 mb-4">
                <h5 class="mb-4">Our Contact</h5>
                <p class="mb-2"><strong>Phone:</strong><br class="d-none d-sm-block">502-555-0152</p>
                <p class="mb-4"><strong>Email:</strong><br class="d-none d-sm-block">brita@firstupconsultants.com</p>
                <div class="social-links">
                    <a href="#" class="me-3 text-dark social-icon"><i class="fab fa-linkedin-in fa-lg"></i></a>
                    <a href="#" class="me-3 text-dark social-icon"><i class="fab fa-instagram fa-lg"></i></a>
                    <a href="#" class="me-3 text-dark social-icon"><i class="fab fa-facebook-f fa-lg"></i></a>
                    <a href="#" class="me-3 text-dark social-icon"><i class="fab fa-twitter fa-lg"></i></a>
                </div>
            </div>
        </div>

        <div class="row mt-4 mt-lg-5">
            <div class="col-12 text-center">
                <p class="mb-4">&copy; {{ date('Y') }} CV. Mandiri Cipta Adikarya. All Rights Reserved.</p>
            </div>
        </div>
    </div>
</footer>
